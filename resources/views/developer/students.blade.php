...
<head>
    ...
    <title>LingoPulse - المطور (الطلاب)</title>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-2 sidebar">
                ...
                <a href="{{ route('admin.students') }}" class="active"><i class="fa-solid fa-user-graduate ms-2"></i> الطلاب</a>
                <a href="{{ route('admin.teachers') }}"><i class="fa-solid fa-chalkboard-user ms-2"></i> المعلمين</a>
            </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div>
                        <button class="btn btn-brand" onclick="showAddModal()">
                            <i class="fa-solid fa-plus ms-1"></i> إضافة طالب جديد
                        </button>
                        <form action="{{ route('admin.autoGroupClasses') }}" method="POST" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-success">
                                <i class="fa-solid fa-wand-magic-sparkles ms-1"></i> تجميع ذكي (AI)
                            </button>
                        </form>
                    </div>
                </div>
                        ...
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $student->email }}</td>
                                    <td>{{ optional($student->student)->level ?? 'غير محدد' }}</td>
                                    <td>
                                        <button class="btn btn-sm btn-outline-primary" onclick="showEditModal({{ json_encode($student) }})">
                                            <i class="fa-solid fa-edit"></i> تعديل
                                        </button>
                                        <form action="{{ route('admin.deleteUser', $student->user_id) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من الحذف؟')">
                                            @csrf
